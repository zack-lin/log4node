	...
	var cpu = {
        ...
        averageTime:200,//如果cpu一直是0%，那就是300ms打一个点
        totalSize:0,//从启动到现在累计的总开销
	}, now, socket = io.connect('http://'+ location.host +'/monitor');
console.log(location.href.replace(location.hash, '')+'/monitor');
	setTimeout(function(){
		now = Date.now();
		cpu.currentTime = now;
        per = ((cpu.currentTime-cpu.lastTime)-200)/200;
        if(per<0){
                per = 0;
        }
        if(per>1){
        ...
        cpu.timerID = setTimeout(arguments.callee,200);
	}, 200);
